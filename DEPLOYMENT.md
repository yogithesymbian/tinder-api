...
4. **Add Environment Variables**:
   ...
   **Important**: Use the **Internal Database URL** from your database's "Connections" page for best performance.
   
   From your database page, copy the internal connection details:
   - Internal Hostname: `tinder-api-db` (just the database name)
   - Username: `tinder_user`
   - Password: (shown in database credentials)
   - Database: `tinder_db`
   ...
   **Note**: The internal hostname is simply your database name (e.g., `tinder-api-db`), not the full external hostname.
...
## PostgreSQL Connection Check Details

### How pg_isready Works with Render.com

The `render-start.sh` script uses PostgreSQL's `pg_isready` utility to verify database availability before starting the application. This works seamlessly with Render's internal database connections:

**What pg_isready does**:
```bash
pg_isready -h "$DB_HOST" -p "${DB_PORT:-5432}" -U "$DB_USERNAME"
```
- Checks if PostgreSQL server is ready to accept connections
- Uses the same hostname that Laravel will use (internal or external)
- Returns immediately with exit code 0 when database is ready
- Lightweight check (doesn't require actual authentication)

**Why it works with Render's internal connections**:
1. The Docker image includes `postgresql-client` package (see Dockerfile)
2. `pg_isready` is part of postgresql-client tools
3. Render's private network resolves internal hostnames (e.g., `tinder-api-db`)
4. The check uses TCP connection, which works over Render's private network

**Configuration**:
- Timeout is configurable via `DB_READY_TIMEOUT` environment variable
- Default: 60 seconds (appropriate for free tier cold starts)
- Logs progress every 5 seconds to avoid log spam
- Provides helpful troubleshooting tips if connection fails

### Common Issues and Solutions

**Issue**: "pg_isready: command not found"
- **Cause**: postgresql-client not installed in Docker image
- **Solution**: Already included in Dockerfile line 16: `postgresql-client`
- **Verification**: The script includes a fallback if pg_isready is missing

**Issue**: "PostgreSQL is not ready after 60 seconds"
- **Possible causes**:
  1. Database hasn't been created in Render dashboard
  2. Using external hostname instead of internal (slower, may timeout)
  3. Database and web service in different regions
  4. Database credentials are incorrect
- **Solutions**:
  1. Verify database exists and is running in Render dashboard
  2. Use internal connection (already configured in render.yaml)
  3. Ensure both services are in the same region
  4. Double-check environment variables match database credentials

**Issue**: "Connection refused" or "Name or service not known"
- **Cause**: Invalid DB_HOST or network configuration
- **Solution**: 
  - For Blueprint deploys: Ensure `render.yaml` uses `fromDatabase` syntax (already configured)
  - For manual deploys: Copy internal connection string from database details
  - Internal hostname format: `<database-name>` (e.g., `tinder-api-db`)
...
#### Understanding Render.com Database Connections

Render.com provides **two types of database connection strings**:

1. **Internal Connection (Recommended for Web Services)**:
   - Format: `<database-name>` (e.g., `tinder-api-db`)
   - Uses Render's private network (faster, no bandwidth charges)
   - Only accessible from services in the same Render account
   - This is what the Blueprint (`render.yaml`) automatically configures

2. **External Connection**:
   - Format: Full hostname with region (e.g., `dpg-xxxxx-a.oregon-postgres.render.com`)
   - Accessible from anywhere
   - Uses public internet (counts against bandwidth limits)

**The `render-start.sh` script works with both internal and external connections** because:
- The `pg_isready` command (included in the Docker image) can resolve internal hostnames
- Render's private network handles DNS resolution for internal connections
- The connection check uses the same `DB_HOST` environment variable that Laravel uses

#### PostgreSQL Readiness Check on Render Free Tier

The startup script includes a configurable timeout for PostgreSQL readiness checks:
- **Default timeout**: 60 seconds (configurable via `DB_READY_TIMEOUT` env var)
- **Why 60 seconds?**: Render's free tier databases spin down after 15 minutes of inactivity
  - Cold start can take 30-60 seconds for the database to wake up
  - The script polls every second but only logs every 5 seconds to reduce noise
- **The script includes**: 
  - Helpful error messages if connection fails
  - Troubleshooting tips for common issues
  - Fallback mechanism if `pg_isready` is not available

**To adjust the timeout** (if needed):
```yaml
# In render.yaml, add or modify:
